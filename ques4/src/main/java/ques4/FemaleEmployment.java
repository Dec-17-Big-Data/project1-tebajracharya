 ...
 * filters out the data for the percentage in change in female employment from year 2000 which is stored back into the HDFS. 
...
public class FemaleEmployment {
	/**
	 * This class is used to read a csv file, set the jar file name to que4.jar and also set
	...
	*/
	public static void main(String[] args) throws Exception {
		...
		Job conf = new Job();
		conf.setJarByClass(FemaleEmployment.class);

		conf.setMapOutputValueClass(Text.class);
		conf.setOutputKeyClass(Text.class);
		conf.setOutputValueClass(DoubleWritable.class);

		conf.setMapperClass(FemaleEmploymentMapper.class);
		conf.setReducerClass(FemaleEmploymentReducer.class);
